...
| `userPrompt` | Carries the message so far, the [message attributes](MessageAttributes.md) and the user's [About Me](AboutMe.md) facts. Supports `{{message}}`, `{{attributes}}` and `{{aboutMe}}`, one field per line -- see [Config.md](devDoc/Config.md#wordprediction). |
