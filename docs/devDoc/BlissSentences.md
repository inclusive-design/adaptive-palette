...
`findGlossEntry(key)` scans the vocabulary as written, then scans it again normalized only if
that first pass missed. The order matters:
...
every candidate is qualified reaches the second pass, which is exactly what normalization is for.

`preferSingleSense` is off on the normalized pass because there every candidate is qualified by
...
Within a pass, a tie goes to the earliest sense position, so `water` is the fluid rather than a late
